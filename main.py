# We'll write the merged `main.py` to /mnt/data so the user can download it.
content = r"""# main.py
import os
import re
import json
...
def resolve_db_path() -> None:
    ...
    try:
        ...
        # re-validate once
        try:
            probe = os.path.join(fallback_dir, ".write_test")
            with open(probe, "w", encoding="utf-8") as f:
                f.write("ok")
            os.remove(probe)
            log.info("DB fallback dir OK: %s", fallback_dir)
        except Exception as e2:
            log.error("DB fallback dir also not writable: %s", e2)
...
(function(){
  ...
  fetch(url)
   .then(async (r) => {
     ...
     let s; try { s = JSON.parse(txt); } catch(e){ throw new Error(\"Invalid JSON: \" + txt.slice(0, 200)); }
     if (typeof s !== \"object\" || s === null) throw new Error(\"Empty payload\");
     const need = [\"btc_dominance\",\"eth_btc\",\"total2_usd\",\"triggers\"]; for (const k of need){ if (!(k in s)) throw new Error(\"Missing key: \" + k); }
     setText(\"alt-asof\", \"As of \" + (s.asof || \"now\") + (s.stale ? \" (cache)\" : \"\"));
...
def _altseason_snapshot(force: bool = False) -> Dict[str, Any]:
    """
    Retourne tjrs un dict. Si toutes les sources échouent, on renvoie le dernier snapshot mémoire
    ou disque, marqué stale=True.
    """
    now = time.time()
...
# -------------------------
# Webhook TradingView (PROTÉGÉ)  — PATCHED to send Telegram rich messages & ignore AOE_*
# -------------------------
@app.post("/tv-webhook")
async def tv_webhook(request: Request, secret: Optional[str] = Query(None)):
    ...
    # 👉 Envoi Telegram pour les signaux de trade (non épinglés) — via builder telegram_rich_message()
    try:
