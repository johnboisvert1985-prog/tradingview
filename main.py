...
async def webhook(request: Request):
    try:
        ...
        try:
            ...
            logger.error(f"❌ JSON invalide après nettoyage")
            logger.error(f"   Body original (300 car): {body_text[:300]}")
            logger.error(f"   Body nettoyé (300 car): {clean_body[:300]}")
            logger.error(f"   Erreur: {str(json_error)}")
            
            return JSONResponse({
                "status": "error", 
                "message": "JSON invalide", 
                "hint": "Vérifiez votre script PineScript - pas de retours à la ligne dans le JSON",
        ...
        if settings.WEBHOOK_SECRET:
            ...
            if received_secret != settings.WEBHOOK_SECRET:
                logger.warning(f"⚠️ Secret invalide reçu")
                return JSONResponse({
                    "status": "error", 
                    "message": "Unauthorized - Invalid secret"
                }, status_code=401)
        
        # Extraire l'action/type
    ...
    try {{
        ...
        trades.forEach(trade => {{
            ...
            if (trade.row_state === 'normal') {{
                ...
                statusBadge = '<span class="badge badge-green">TP1 OK</span>';
            }} else if (trade.row_state === 'tp2') {{
                statusBadge = '<span class="badge badge-green">TP2 OK</span>';
            }} else if (trade.row_state === 'tp3') {{
                statusBadge = '<span class="badge badge-green">TP3 OK</span>';
