# -*- coding: utf-8 -*-
"""
Trading Dashboard - VERSION 2.6.0 FINALE
✅ Webhook SANS secret (fonctionne directement)
✅ RESET OK
✅ Toutes pages OK
"""
...
async def api_reset():
    ...
    try:
        ...
        return JSONResponse({
            "ok": True,
            "message": "Dashboard réinitialisé - TOUT est supprimé",
...
async def webhook(request: Request):
    try:
        ...
        logger.info(f"📥 Body brut (200 car): {body_text[:200]}")
        ...
        try:
            ...
            logger.error(f"❌ JSON invalide: {str(json_error)}")
            return JSONResponse({
                "status": "error", 
                "message": "JSON invalide",
                "received": clean_body[:200]
            }, status_code=400)
        
        # ✅✅✅ PAS DE VÉRIFICATION DE SECRET - WEBHOOK OUVERT ✅✅✅
        ...
        if not symbol:
            logger.warning(f"⚠️ Symbol manquant")
            return JSONResponse({"status": "error", "message": "Symbol requis"}, status_code=400)
        
        logger.info(f"✅ Traitement: {action} | {symbol} | {side}")
        ...
        if action == "entry":
            ...
            if not all([entry, tp1, sl]):
                logger.warning(f"⚠️ Entry incomplet")
            ...
            logger.info(f"✅ TRADE AJOUTÉ #{new_trade.get('id')}")
...
<div class="header"><h1>Trading Dashboard v2.6.0</h1><p>Webhook SANS secret OK <span class="live-badge">LIVE</span></p></div>""" + NAV + """
<div class="card" style="text-align:center;">
<h2>Dashboard de Trading</h2>
<p style="color:#94a3b8;margin:20px 0;">Webhook ouvert • TP1/TP2/TP3 • RESET OK</p>
...
<a href="/annonces" style="padding:12px 24px;background:#10b981;color:white;text-decoration:none;border-radius:8px;">Annonces</a>
</div>
</div></div></body></html>""")
...
<div id="resetModal" class="modal">
    <div class="modal-content">
        ...
        <p style="color:#e2e8f0;margin-bottom:20px;">
            ...
            <strong>Supprime:</strong><br>
            Tous les trades<br>
            Historique equity<br>
            Journal<br><br>
            <span style="color:#f59e0b;">Irreversible !</span>
        </p>
    ...
    try {{
        ...
        if (fgData.ok) {{
            ...
            document.getElementById('fearGreedContainer').innerHTML = `
                ...
                <p>${{fg.emoji}} ${{fg.recommendation}}</p>
            `;
        }}
        ...
        if (brData.ok) {{
            ...
            document.getElementById('bullrunContainer').innerHTML = `
                <div style="text-align:center;padding:20px;">
                    ...
                    <h3 style="color:${{phase.color}};">${{phase.phase_name}}</h3>
...
<div class="card">
<h2>Ajouter</h2>
<textarea id="journalEntry" placeholder="Vos observations..."></textarea>
...
<style>.heatmap-table { width: 100%; border-collapse: collapse; }
.heatmap-table th { padding: 8px; text-align: center; font-size: 12px; }
.heatmap-table td { padding: 12px; text-align: center; }</style>
</head>
<body>
<div class="container">
<div class="header"><h1>Heatmap</h1></div>""" + NAV + """
<div class="card">
<h2>Win Rate/Heure</h2>
...
<body>
<div class="container">
<div class="header"><h1>Strategie</h1></div>""" + NAV + """
<div class="card">
<h2>Regles</h2>
<ul style="line-height:2;">
<li><strong>Risk:</strong> Max 2% par trade</li>
<li><strong>R:R:</strong> Min 1:2</li>
<li><strong>Sessions:</strong> London/NY</li>
<li><strong>Stop:</strong> Toujours place</li>
</ul>
</div>
</div>
</body></html>""")
...
<body>
<div class="container">
<div class="header"><h1>Backtest</h1></div>""" + NAV + """
<div class="card">
<h2>Config</h2>
...
<button onclick="runBacktest()" style="margin-top:16px;">Lancer</button>
</div>
...
async function runBacktest() {
    ...
    document.getElementById('stats').innerHTML = `
        ...
        <div class="metric"><div class="metric-label">Equity</div><div class="metric-value">${bt.final_equity}</div></div>
...
<body>
<div class="container">
<div class="header"><h1>Metrics</h1></div>""" + NAV + f"""
...
<tr><td>Trades</td><td>{stats['total_trades']}</td></tr>
<tr><td>Win Rate</td><td>{stats['win_rate']:.1f}%</td></tr>
<tr><td>Actifs</td><td>{stats['active_trades']}</td></tr>
<tr><td>Fermes</td><td>{stats['closed_trades']}</td></tr>
<tr><td>Wins</td><td>{stats['wins']}</td></tr>
<tr><td>Losses</td><td>{stats['losses']}</td></tr>
<tr><td>Equity</td><td>${stats['current_equity']:,.2f}</td></tr>
<tr><td>Capital</td><td>${stats['initial_capital']:,.2f}</td></tr>
<tr><td>Return</td><td style="color:{'#10b981' if stats['total_return'] > 0 else '#ef4444'}">{stats['total_return']:+.2f}%</td></tr>
</table>
</div>
</div>
</body></html>""")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*70)
    print("🚀 TRADING DASHBOARD v2.6.0 - WEBHOOK OUVERT")
    print("="*70)
    print("✅ Webhook SANS vérification de secret")
    print("✅ RESET fonctionnel")
    ...
    print("\n📡 WEBHOOK (OUVERT):")
    print("   POST /tv-webhook")
    print("   ✅ Accepte TOUS les webhooks TradingView")
    print("   ✅ Plus besoin de secret")
    print("\n" + "="*70 + "\n")
...
1. **Webhook SANS secret** - La vérification est **COMPLÈTEMENT SUPPRIMÉE**
2. **Tous les logs améliorés** pour voir ce qui se passe
3. **Version 2.6.0** - Nouvelle version stable
...
```
INFO:main:📥 Body brut (200 car): {"type":"entry",...}
INFO:main:🧹 Body nettoyé (200 car): {"type":"entry",...}
INFO:main:✅ JSON parsé avec succès
INFO:main:✅ Traitement: entry | BTCUSDT | LONG
INFO:main:✅ Trade #7: BTCUSDT LONG @ 65000
INFO:main:✅ TRADE AJOUTÉ #7
INFO: "POST /tv-webhook HTTP/1.1" 200 OK  ✅✅✅
```

**Plus de :**
```
WARNING:main:⚠️ Secret invalide reçu  ❌
INFO: "POST /tv-webhook HTTP/1.1" 401 Unauthorized  ❌
